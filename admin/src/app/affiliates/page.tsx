...
import {
  HandCoins, RefreshCw, Users, Banknote, CheckCircle2, XCircle, Clock, Percent, Wallet,
} from 'lucide-react';
...
export default function AdminAffiliatesPage() {
  ...
  }, [tab, commissionFilter, payoutFilter]);
  ...
  const loadData = async () => {
    try {
      ...
      if (tab === 'affiliates') {
        ...
        setPayouts(await api.getAdminPayouts(payoutFilter));
      } else {
  ...
  const handlePayoutPay = async (id: string) => {
    try {
      setActionId(id);
  ...
  const handlePayoutReject = async (id: string) => {
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 py-6 space-y-5">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 fade-up">
          ...
          <div className="flex items-center gap-1.5">
            {[
              ...
              { id: 'payouts', label: 'Requests', icon: Wallet },
        ...
        {tab === 'payouts' && (
          <div className="flex flex-wrap items-center gap-2 fade-up">
            <div className="glass rounded-xl px-3.5 py-2 flex items-center gap-2 text-[11px]">
              <Clock className="w-3.5 h-3.5 text-orange-400" />
              <span className="text-slate-400 font-bold">Pending requests:</span>
              <span className="font-black text-orange-300">
                {payouts.filter((p) => p.status === 'PENDING').reduce((s, p) => s + p.amount, 0).toLocaleString()} ETB
              </span>
            </div>
            <div className="flex items-center gap-1.5 ml-auto">
          ...
          <div className="glass rounded-3xl overflow-hidden fade-up">
            <div className="overflow-x-auto">
              <table className="w-full table-base">
                ...
                <tbody>
                  {payouts.length === 0 && (
                    <tr><td colSpan={7} className="text-center text-slate-500 py-8">No payout requests</td></tr>
                  )}
                  {payouts.map((p) => (
                    <tr key={p.id}>
                      <td>
                        <div className="font-bold text-slate-100">{p.affiliate?.name}</div>
                        {p.affiliate?.telegramUsername && (
                          <div className="text-[10px] text-sky-400">@{p.affiliate.telegramUsername}</div>
                        )}
                        <div className="text-[10px] text-amber-400 font-mono">{p.affiliate?.code}</div>
                      </td>
                      <td className="font-black text-emerald-400 whitespace-nowrap">
                        {p.amount.toLocaleString()} ETB
                      </td>
                      <td>
                        <span className="inline-flex items-center gap-1.5 px-2 py-1 rounded-lg bg-white/[0.04] border border-white/10">
                          <span className="w-5 h-5 rounded bg-white flex items-center justify-center p-0.5 shrink-0">
                            <img
                              src={p.method === 'telebirr' ? '/payments/telebirr.svg' : '/payments/cbe-birr.svg'}
                              alt={p.method}
                              className="max-w-full max-h-full object-contain"
                            />
                          </span>
                          <span className="text-[10px] font-bold text-slate-300 uppercase">
                            {p.method === 'telebirr' ? 'Telebirr' : 'CBE Birr'}
                          </span>
                        </span>
                      </td>
                      <td className="font-mono text-[11px] text-slate-200">{p.account}</td>
                      <td>
                      ...
                      <td>
                        ...
                        {p.status === 'PAID' && (
                          <span className="text-[9px] text-slate-500">{new Date(p.updatedAt).toLocaleDateString()}</span>
                        )}
                      </td>
                    </tr>
                  ))}
